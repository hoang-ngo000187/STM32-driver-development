...
int main(void)
{
  ...
  /* 01 - Enable the HSE clock using HSEON bit (RCC_CR) */
  *pRccCrReg |= (1 << 16);

  /* 02 - Wait until HSE clock form the external crystal stabilizes (only if crystal is connected) */
  while( !((*pRccCrReg) & (1 << 17)) );

  /* 03 - Switch the system clock to HSE (RCC_CFGR) */
  *pRccCfgrReg |= (1 << 0);


  /******************************************** 04 - Do MCO1 settings to measure it ********************************************/
  /* 01) Bit22 = 1 and Bit21 = 0 to select HSE clock source for MCO1 signal */
	*pRccCfgrReg |= (1 << 22);
